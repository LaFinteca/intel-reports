// Generates static manifests — reports.json (from reports/) and digests.json
// (from digests/). Each entry: { name, size (bytes), created (ISO, from the
// file's first git commit) }. Sorted newest-first. The site reads these
// instead of the GitHub API, so the lists never break under API rate limits.
//
...
function buildManifest(dir, out) {
  if (!existsSync(dir)) {
  ...
  console.log(`Wrote ${out} with ${entries.length} file(s):`);
  for (const e of entries) console.log(`  ${e.created.slice(0, 10)}  ${e.name}`);
}

buildManifest("reports", "reports.json");
buildManifest("digests", "digests.json");
